...
// Define match conditions for necklace filtering
...
async function handleRequest(request: NextRequest) {
  try {
    ...
    console.log('API Request for necklace category:', category);
    ...
    if (category !== 'all') {
      ...
      } else if (category.startsWith('length-')) {
        const length = category
          .replace('length-', '')
          .split('-')
          .map(word => word.charAt(0).toUpperCase() + word.slice(1))
          .join(' ');
        matchConditions.length = length;
      }
    }
    ...
    if (lengths?.length) {
      delete matchConditions.length;
      matchConditions.length = { $in: lengths };
    }
    ...
    const [result] = await Necklace.aggregate(pipeline);
    const totalCount = result.metadata[0]?.total || 0;
    
    console.log(`Found ${totalCount} necklaces matching the criteria`);
    ...
    return NextResponse.json(
      { error: 'Failed to fetch necklaces' },
      { status: 500 }
    );
  }
}
