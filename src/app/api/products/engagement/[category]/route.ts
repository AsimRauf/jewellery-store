...
async function handleRequest(request: NextRequest) {
  try {
    ...
    // Fetch products with pagination - include metalColorImages and slug in the selection
    const products = await EngagementRing.find(query)
      .select('_id slug title SKU basePrice metalOptions metalColorImages media main_stone style type')
      .sort(sortOptions)
      .skip(skip)
      .limit(limit)
      .lean();
