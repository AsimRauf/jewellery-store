...
// Define match conditions for bracelet filtering
interface MatchConditions {
  isAvailable: boolean;
  type?: string | { $in: string[] };
  metal?: string | { $in: string[] };
  style?: string | { $in: string[] };
  closure?: string | { $in: string[] };
...
async function handleRequest(request: NextRequest) {
  try {
    ...
    console.log('API Request for bracelet category:', category);
    ...
    if (category !== 'all') {
      ...
      } else if (category.startsWith('closure-')) {
        const closure = category
          .replace('closure-', '')
          .split('-')
          .map(word => word.charAt(0).toUpperCase() + word.slice(1))
          .join(' ');
        matchConditions.closure = closure;
      }
    }
    ...
    if (closures?.length) {
      delete matchConditions.closure;
      matchConditions.closure = { $in: closures };
    }

    // Length filtering (in inches)
    ...
    const [result] = await Bracelet.aggregate(pipeline);
    const totalCount = result.metadata[0]?.total || 0;
    
    console.log(`Found ${totalCount} bracelets matching the criteria`);
