...
export default function EngagementRingsList() {
  const { user } = useUser();
  
  const [rings, setRings] = useState<EngagementRing[]>([]);
  ...
  const fetchRings = async () => {
    try {
      ...
      const response = await fetch(`/api/admin/rings/engagement?${queryParams}`, {
        credentials: 'include'
      });
      ...
      toast.error('Failed to fetch engagement rings');
  ...
  return (
    <div className="max-w-7xl mx-auto p-4 sm:p-6 lg:p-8">
      ...
      <div className="flex flex-col sm:flex-row justify-between items-start sm:items-center mb-6">
        <div>
          <h1 className="text-3xl font-bold text-gray-900">Engagement Rings</h1>
          <p className="mt-2 text-gray-600">Manage your engagement ring collection</p>
        </div>
        ...
        <div className="flex flex-col sm:flex-row gap-4">
          <div className="flex-1">
            <div className="relative">
              ...
              <input
                type="text"
                placeholder="Search rings by title, SKU, or stone type..."
                value={search}
      ...
      {isLoading ? (
        ...
        <div className="bg-white rounded-lg shadow-sm border border-gray-200 p-8 text-center">
          ...
          <h3 className="text-lg font-medium text-gray-900 mb-2">No engagement rings found</h3>
          <p className="text-gray-600 mb-4">Get started by adding your first engagement ring.</p>
          <Link
            href="/admin/rings/engagement"
                  ...
                  <tr key={ring._id} className="hover:bg-gray-50">
                        ...
                        <div className="flex-shrink-0 h-16 w-16">
                          {ring.media?.images?.[0]?.url ? (
                            <Image
                              ...
                              alt={ring.title || 'Engagement ring'}
                              width={64}
                              height={64}
                    ...
                    <td className="px-6 py-4 whitespace-nowrap">
                      <div className="text-sm text-gray-900">
                        {ring.style?.length > 0 && (
                          <div className="mb-1">
                            Style: {ring.style.slice(0, 2).join(', ')}
                            {ring.style.length > 2 && ` +${ring.style.length - 2} more`}
                          </div>
                        )}
                        {ring.type?.length > 0 && (
                          <div>
                            Type: {ring.type.slice(0, 2).join(', ')}
                            {ring.type.length > 2 && ` +${ring.type.length - 2} more`}
                          </div>
                        )}
                      </div>
                    ...
                    <td className="px-6 py-4 whitespace-nowrap">
                      <div className="text-sm text-gray-900">
                        {ring.main_stone?.type && (
                          <div className="mb-1">
                            {ring.main_stone.type}
                            {ring.main_stone.gemstone_type && ` (${ring.main_stone.gemstone_type})`}
                          </div>
                        )}
                        {ring.main_stone?.carat_weight > 0 && (
                          <div className="text-xs text-gray-500">
                            {ring.main_stone.carat_weight}ct {ring.main_stone.shape} {ring.main_stone.color}
                          </div>
                        )}
                        {ring.main_stone?.number_of_stones > 0 && (
                          <div className="text-xs text-gray-500">
                            {ring.main_stone.number_of_stones} stone{ring.main_stone.number_of_stones > 1 ? 's' : ''}
                          </div>
                        )}
                      </div>
                    </td>
                    ...
                    <td className="px-6 py-4 whitespace-nowrap text-sm font-medium">
                      <div className="flex items-center gap-2">
                        <Link
                          href={`/admin/rings/engagement/${ring._id}`}
                        ...
                        <Link
                          href={`/admin/rings/engagement/edit/${ring._id}`}
