...
export default function EditBraceletPage({ params }: { params: Promise<{ id: string }> }) {
  ...
  const fetchBracelet = useCallback(async () => {
    try {
      setLoading(true);
      const response = await fetch(`/api/admin/bracelets/${resolvedParams.id}`);
      ...
      setFormData({
        ...
        name: data.name || '',
        type: data.type || Object.values(BraceletType)[0],
        closure: data.closure || Object.values(BraceletClosure)[0],
        metal: data.metal || Object.values(BraceletMetal)[0],
        style: data.style || Object.values(BraceletStyle)[0],
        length: data.length || 0,
        width: data.width || 0,
        adjustable: data.adjustable || false,
      ...
      toast.error('Failed to fetch bracelet data');
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      if (!response.ok) {
        ...
        throw new Error(errorData.error || 'Failed to update bracelet');
      }
      ...
      if (result.success) {
        ...
        throw new Error(result.error || 'Failed to update bracelet');
      }
    } catch (error) {
      console.error('Error updating bracelet:', error);
      toast.error(error instanceof Error ? error.message : 'Failed to update bracelet');
    } finally {
      setSaving(false);
    }
  };
  ...
  const handleDelete = async () => {
    if (!confirm('Are you sure you want to delete this bracelet? This action cannot be undone.')) {
      return;
    }
    ...
    try {
      ...
      if (!response.ok) {
        const errorData = await response.json();
        throw new Error(errorData.error || 'Failed to delete bracelet');
      }
      ...
      toast.error(error instanceof Error ? error.message : 'Failed to delete bracelet');
    } finally {
      setDeleting(false);
    }
  };
  ...
  if (!bracelet) {
    return (
      <div className="min-h-screen flex items-center justify-center">
        <div className="text-center">
          <h1 className="text-2xl font-bold text-gray-900 mb-4">Bracelet Not Found</h1>
          <Link href="/admin/bracelets/list" className="text-indigo-600 hover:text-indigo-800">
            Back to Bracelets List
          </Link>
        </div>
      </div>
    );
    ...
    <div className="min-h-screen bg-gray-50 py-8">
      <div className="max-w-4xl mx-auto px-4 sm:px-6 lg:px-8">
        <div className="mb-8">
          <div className="flex items-center justify-between mb-6">
            <div className="flex items-center">
              ...
              <h1 className="text-3xl font-bold text-gray-900">Edit Bracelet</h1>
            </div>
            <button
        ...
        <form onSubmit={handleSubmit} className="space-y-8">
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
              ...
              <div>
                ...
                <select
                  ...
                  required
                >
                  {Object.entries(BraceletType).map(([key, value]) => (
              ...
              <div>
                ...
                <select
                  ...
                  required
                >
                  {Object.entries(BraceletClosure).map(([key, value]) => (
                    <option key={key} value={value}>{value}</option>
                  ))}
                </select>
              </div>
              ...
              <div>
                ...
                <select
                  ...
                  required
                >
                  {Object.entries(BraceletMetal).map(([key, value]) => (
                    <option key={key} value={value}>{value}</option>
                  ))}
                </select>
              </div>
              ...
              <div>
                ...
                <select
                  ...
                  required
                >
                  {Object.entries(BraceletStyle).map(([key, value]) => (
                    <option key={key} value={value}>{value}</option>
                  ))}
                </select>
              </div>
          ...
          {/* Bracelet Details */}
          <div className="bg-white shadow rounded-lg p-6">
            <h2 className="text-lg font-medium text-gray-900 mb-6">Bracelet Details</h2>
          ...
          <div className="bg-white shadow rounded-lg p-6">
            ...
            <div>
              <textarea
                ...
                rows={4}
                className="w-full px-3 py-2 border border-gray-300 rounded-md focus:outline-none focus:ring-2 focus:ring-indigo-500"
                placeholder="Enter bracelet description..."
              />
            </div>
          </div>
          ...
          <div className="bg-white shadow rounded-lg p-6">
            ...
            <div>
              <textarea
                ...
                rows={3}
                className="w-full px-3 py-2 border border-gray-300 rounded-md focus:outline-none focus:ring-2 focus:ring-indigo-500"
                placeholder="Enter features separated by commas (e.g., Hypoallergenic, Adjustable, Water-resistant)"
              />
            </div>
          </div>
          ...
          <div className="flex items-center justify-end space-x-4">
            ...
            >
              {saving ? 'Updating...' : 'Update Bracelet'}
            </button>
          </div>
        </form>
      </div>
