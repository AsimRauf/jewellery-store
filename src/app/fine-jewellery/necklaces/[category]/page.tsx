...
import FilterBar from '@/components/necklace/FilterBar';
import MobileFilters from '@/components/necklace/MobileFilters';
import SortingOptions from '@/components/necklace/SortingOptions';
import ProductGrid from '@/components/necklace/ProductGrid';
...
const availableFilters = {
  ...
  chainWidths: ['Thin', 'Medium', 'Thick', 'Extra Thick'],
  claspTypes: ['Lobster', 'Spring Ring', 'Toggle', 'Magnetic', 'Hook', 'Ball Clasp'],
  priceRanges: [[50, 200], [200, 500], [500, 1000], [1000, 2500], [2500, 5000], [5000, 10000], [10000, 25000]] as [number, number][]
};

export default function NecklaceCategoryPage() {
  ...
  const [filters, setFilters] = useState({
    ...
    metals: [] as string[],
    styles: [] as string[],
    chainWidths: [] as string[],
    claspTypes: [] as string[],
  ...
  // State for products
  const [products, setProducts] = useState<Necklace[]>([]);
  ...
  useEffect(() => {
    const initFiltersFromUrl = () => {
      ...
      const chainWidths = searchParams.get('chainWidths')?.split(',') || [];
      const claspTypes = searchParams.get('claspTypes')?.split(',') || [];
      ...
      setFilters({
        ...
        chainWidths: chainWidths.filter(c => availableFilters.chainWidths.includes(c)),
        claspTypes: claspTypes.filter(c => availableFilters.claspTypes.includes(c)),
  ...
  useEffect(() => {
    const fetchProducts = async () => {
      try {
        ...
        if (filters.chainWidths.length > 0) {
          queryParams.append('chainWidths', filters.chainWidths.join(','));
        }
        
        if (filters.claspTypes.length > 0) {
          queryParams.append('claspTypes', filters.claspTypes.join(','));
        }
        ...
        // Fetch products
        const response = await fetch(`/api/products/necklace/${category}?${queryParams.toString()}`);
  ...
  const loadMoreProducts = useCallback(async () => {
    ...
    try {
      ...
      const data = await response.json();
      setProducts((prev: Necklace[]) => [...prev, ...data.products]);
      setPage(nextPage);
      setHasMore(data.page < data.totalPages);
  ...
  const updateUrlWithFilters = useCallback(() => {
    ...
    // Update URL
    const url = `/fine-jewellery/necklaces/${category}?${queryParams.toString()}`;
  ...
  const toggleChainWidth = (chainWidth: string) => {
    setFilters(prev => ({
      ...prev,
      chainWidths: prev.chainWidths.includes(chainWidth)
        ? prev.chainWidths.filter(c => c !== chainWidth)
        : [...prev.chainWidths, chainWidth]
    }));
  };
  
  const toggleClaspType = (claspType: string) => {
    setFilters(prev => ({
      ...prev,
      claspTypes: prev.claspTypes.includes(claspType)
        ? prev.claspTypes.filter(c => c !== claspType)
        : [...prev.claspTypes, claspType]
    }));
  };
  ...
  const getCategoryDisplayName = () => {
    if (!category || category === 'all') return 'All Necklaces';
    
    // Handle specific necklace types
    if (availableFilters.types.some(type => 
      category.toLowerCase() === type.toLowerCase()
    )) {
      return `${category.charAt(0).toUpperCase() + category.slice(1)} Necklaces`;
    }
    ...
    if (category.startsWith('style-')) {
      ...
      return `${style} Necklaces`;
    }
    
    return 'Necklaces';
  };

  const handleNecklaceClick = (necklace: Necklace) => {
    const productSlug = necklace.slug || necklace._id;
    router.push(`/fine-jewellery/necklaces/detail/${productSlug}`);
  };
    ...
    <div className="container mx-auto px-4 py-8">
      ...
      <div className="mb-8">
        ...
        <p className="text-gray-600 mt-2">
          {totalProducts} {totalProducts === 1 ? 'necklace' : 'necklaces'} available
        </p>
      </div>
      ...
      <div className="hidden lg:block mb-8">
        <div className="bg-white border border-gray-200 rounded-lg p-4 shadow-sm">
          <FilterBar
            ...
            toggleChainWidth={toggleChainWidth}
            toggleClaspType={toggleClaspType}
      ...
      <div>
        ...
        <ProductGrid
          ...
          onProductClick={handleNecklaceClick}
